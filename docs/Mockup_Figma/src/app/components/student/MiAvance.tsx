...
import { CheckCircle2, Clock, Circle, Download, FileText, AlertTriangle, Info } from "lucide-react";
...
function StatusIcon({ estado }: { estado: string }) {
  if (estado === "Aprobado") return <CheckCircle2 size={22} color="#22c55e" style={{ flexShrink: 0 }} />;
  if (estado === "Cursando") return <Clock size={22} color="#f59e0b" style={{ flexShrink: 0 }} />;
...
export function MiAvance() {
  const [activeTab, setActiveTab] = useState("Resumen");
  const progreso = 62;

  return (
    <>
      <PageHeader title="5. PANTALLA DE PERFIL ESTUDIANTIL" />
              ...
              <>
                ...
                <div style={{ display: "flex", gap: "12px", marginTop: "24px" }}>
                  <button style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 18px", backgroundColor: "#0d2035", color: "white", border: "none", borderRadius: "6px", fontSize: "13px", fontWeight: 500, cursor: "pointer" }}>
                    <Download size={14} /> Descargar Certificado Analítico
                  </button>
